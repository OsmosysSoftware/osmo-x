...
<Steps>
  ...
  <Step title="Retry on failure">
    A failed attempt is logged as `Failed` — a factual record of that attempt. If attempts remain, a next one is re-queued with a delay of `WEBHOOK_RETRY_INTERVAL`, and the webhook's `last_delivery_status` (not the attempt log) shows `Retrying` in the meantime.
  </Step>
  <Step title="Give up after max attempts">
    Once `WEBHOOK_MAX_RETRY_COUNT` attempts have been made, `last_delivery_status` moves to `Failed` too and no further attempts are made.
  </Step>
</Steps>
