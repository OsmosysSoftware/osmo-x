...
<Note>
  All endpoints above are scoped to the caller's own organization by default. A `SUPER_ADMIN` can target a different org via `?organization_id=<id>`; any other role passing an `organization_id` other than their own gets a `400 Bad Request`.
</Note>
...
<Steps>
  ...
  <Step title="Retry on failure">
    If the attempt fails and attempts remain, it's logged with status `Retrying` and re-queued with a delay of `WEBHOOK_RETRY_INTERVAL`.
  </Step>
  <Step title="Give up after max attempts">
    Once `WEBHOOK_MAX_RETRY_COUNT` attempts have been made, the delivery is logged as `Failed` and no further attempts are made.
  </Step>
</Steps>
